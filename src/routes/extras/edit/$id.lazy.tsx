...
const UpdateForm: React.FC<{
  ...
  const onSubmit = async (data: Validator) => {
    try {
      ...
      await network().post("/developer/update-extra", { ...data, id: props.id });
      navigate({ to: "/extras" });
    } catch (e) {
      console.log(e);
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center px-4 py-2">
        ...
        <Input {...register("price", { valueAsNumber: true })} className="w-full" type="number" />
      </div>
...
const Page: React.FC = () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await network().get(`/developer/extras/${id}`);
...
export const Route = createLazyFileRoute("/extras/edit/$id")({
  component: Page,
});
