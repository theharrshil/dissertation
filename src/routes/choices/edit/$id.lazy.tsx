...
const UpdateForm: React.FC<{
  ...
  const onSubmit = async (data: Validator) => {
    try {
      ...
      await network().post("/developer/update-choice", { ...data, id: props.id });
      navigate({ to: "/choices" });
    } catch (e) {
      console.log(e);
    }
...
const Page: React.FC = () => {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await network().get(`/developer/choices/${id}`);
  ...
  return (
    <div className="max-h-screen overflow-y-scroll">
      ...
      {data && (
        <div className="p-3">
          <UpdateForm bhk={data.bhk} name={data.name} image={data.image} id={id} />
        </div>
      )}
    </div>
  );
...
export const Route = createLazyFileRoute("/choices/edit/$id")({
  component: Page,
});
